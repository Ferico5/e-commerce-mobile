      ...
      {isLoading ? (
        ...
        <>
          {visibleOrders.map((order) => (
            <View
              key={order._id}
              className="flex-row w-full border-b border-t border-[#E5E7EB] py-5"
            >
              {/* image */}
              <View className="relative w-[88px] h-[160px] mr-3">
                <Image
                  source={{ uri: order.items[0].image[0] }}
                  className="w-[80px] h-full rounded border border-gray-200 z-10 relative"
                  resizeMode="cover"
                />
                {/* if product length > 1 */}
                {order.items.length > 1 && (
                  <Image
                    source={{ uri: order.items[1].image[0] }}
                    className="w-[80px] h-full rounded border border-gray-200 absolute top-2 left-2 z-0 opacity-80"
                    resizeMode="cover"
                  />
                )}
              </View>
              ...
              <View className="flex">
                <View className="flex-row items-center mb-2">
                  <Text className="font-outfit text-lg w-[75%]">
                    {order.items[0].name}
                  </Text>
                  {order.items.length > 1 && (
                    <Text className="bg-gray-200 text-xs px-2 py-1 rounded-full font-outfit">
                      +{order.items.length - 1}
                    </Text>
                  )}
                </View>
                ...
                <Text className="font-outfit text-sm mb-1">
                  Date:{" "}
                  <Text className="font-outfit text-[#A2A9B4]">
                ...
                <Text className="font-outfit text-sm mb-1">
                  Payment:{" "}
                  <Text className="font-outfit text-[#A2A9B4]">
                    Bank {order.paymentMethod.toUpperCase()}
                  </Text>
                </Text>
                ...
                <View className="flex-row items-center justify-between w-[85%]">
                  ...
                  <View>
                    <Pressable
                      ...
                      className="border border-[#E5E7EB] px-4 py-2 self-start"
                    >
