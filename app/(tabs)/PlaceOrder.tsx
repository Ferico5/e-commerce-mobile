...
export default function PlaceOrder() {
  ...
  const updateField = <K extends keyof OrderFormValues>(
    key: K,
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      <TextInput
        value={name}
        className="w-full h-12 px-3 rounded-md border border-[#D1D5DB] mb-4 font-outfit"
        editable={false}
      ...
      />
      {errors.street && (
        <Text className="text-red-500 text-sm mb-2 ml-3 font-outfit">
          {errors.street}
        </Text>
      )}
      ...
      <View className="flex-row gap-3">
        <View className="flex-1">
          <TextInput
            ...
            placeholder="City"
            placeholderTextColor="#9CA3AF"
            className={`h-12 px-3 rounded-md border border-[#D1D5DB] ${errors.city ? "mb-2" : "mb-4"} font-outfit`}
          />
          {errors.city && (
            <Text className="text-red-500 text-sm mb-2 ml-3 font-outfit">
              {errors.city}
      ...
      <View>
        ...
        <Pressable
          ...
          className={`px-6 py-3 mt-5 self-end ${cart.length === 0 || createOrderMutation.isPending ? "bg-gray-400" : "bg-black"}`}
        >
